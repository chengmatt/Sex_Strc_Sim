# Purpose: To conduct a sensitivity run to look at the impact of increased uncertainty
# in catch data
...
# Create directory for experiment 2 catch sensitivity runs and move OMs around to run in 
# a separate folder
dir.create(here("output", "Experiment 2 Catch Sensitivity"))
dir.create(here("output", "Experiment 2 Catch Sensitivity", "Grwth15_Mort0"))
dir.create(here("output", "Experiment 2 Catch Sensitivity", "Grwth15_Mort15"))
dir.create(here("output", "Experiment 2 Catch Sensitivity", "Grwth0_Mort15"))
dir.create(here("output", "Experiment 2 Catch Sensitivity", "Grwth0_Mort0"))

# Move OMs to new sensitvity folder
file.copy(from = here("output", "Experiment 2", "Grwth15_Mort0", "Grwth15_Mort0.RData"),
          to = here("output", "Experiment 2 Catch Sensitivity", "Grwth15_Mort0"))
file.copy(from = here("output", "Experiment 2", "Grwth15_Mort15", "Grwth15_Mort15.RData"),
          to = here("output", "Experiment 2 Catch Sensitivity", "Grwth15_Mort15"))
file.copy(from = here("output", "Experiment 2", "Grwth0_Mort15", "Grwth0_Mort15.RData"),
          to = here("output", "Experiment 2 Catch Sensitivity", "Grwth0_Mort15"))
file.copy(from = here("output", "Experiment 2", "Grwth0_Mort0", "Grwth0_Mort0.RData"),
          to = here("output", "Experiment 2 Catch Sensitivity", "Grwth0_Mort0"))

# Set up paths to OMs
om_path <- here("output", "Experiment 2 Catch Sensitivity")
...
ems_exp2 <- read_xlsx(here("input", "run_EMs.xlsx"), sheet = "EM_Exp2", na = "NA") %>% 
  mutate(EM_Name = paste(EM_Name, "Catch_LargErr")) # rename some EMs to explicitly state more error
