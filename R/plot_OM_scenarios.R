...
srvageselex_plot = ggplot(srvageselex_all %>% 
                            filter(OM == "Across_100"), aes(x = Age, y = Value, color = factor(Sex))) +
  geom_line(size = 2, alpha = 0.8) +
...
naa_plot = ggplot(naa_sum %>% 
                    filter(OM == "Across_100"), 
                  aes(x = Years, y = Median, ymin = lwr_95, ymax = upr_95, color = factor(Sex),
                               fill = factor(Sex))) +
  geom_line(size = 1.5) +
  geom_ribbon(alpha = 0.35, color = NA) +
  labs(x = "Years", y = "", color = "Sex", fill = "Sex", 
...
ggarrange(vonB_plot, waa_plot, fishageselex_plot, 
          ...
          naa_plot, catch_plot, ssb_plot, nrow = 2, ncol = 4)
dev.off()
