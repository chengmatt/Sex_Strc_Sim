  ...
  for(sim in 1:n_sims) {
    ...
    em_inputs = prepare_EM_inputs(sim = sim,
                                  ...
                                  # Biologicals
                                  WAA = as.matrix(waa[,1]),
                                  age_len_transition = array(al_matrix[,,1], dim = c(30, 30, 1)),
    ...
    models = run_model(data = em_inputs$data, 
                       ...
                       map = em_inputs$map, silent = T, n.newton = 4)
    ...
    if(sum(is.na(models$sd_rep$sd)) == 0) {
      ...
      Req_est = get_Req(SBPR_Fmsy = SBPR_MSY_est, waa = waa[,1], mat_at_age = mat_at_age[,1], ages = age_bins)
      ...
      est_hcr_catch[sim] =  get_proj_catch(fmsy_val = fmsy_val,
                                    ...
                                    WAA = biologicals$waa_nosex, MatAA = mat_at_age)
