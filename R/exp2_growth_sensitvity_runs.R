# Purpose: To conduct a sensitivity run to look at the impact of mis-specifying growth
...
# Create directory for experiment 2 growth sensitivity runs and move OMs around to run in 
# a separate folder
dir.create(here("output", "Experiment 2 Growth Sensitivity"))
dir.create(here("output", "Experiment 2 Growth Sensitivity", "Grwth15_Mort0"))
dir.create(here("output", "Experiment 2 Growth Sensitivity", "Grwth15_Mort15"))

# Move OMs to new sensitvity folder
file.copy(from = here("output", "Experiment 2", "Grwth15_Mort0", "Grwth15_Mort0.RData"),
          to = here("output", "Experiment 2 Growth Sensitivity", "Grwth15_Mort0"))
file.copy(from = here("output", "Experiment 2", "Grwth15_Mort15", "Grwth15_Mort15.RData"),
          to = here("output", "Experiment 2 Growth Sensitivity", "Grwth15_Mort15"))

# Set up paths to OMs
om_path <- here("output", "Experiment 2 Growth Sensitivity")
om_names <- list.files(om_path)

# Read in OMs for experiment 2 to use for this sensitivity run
oms_exp2 <- read_xlsx(here("input", "generate_OMs.xlsx"), sheet = "OM_Exp2", na = "NA") %>% filter(str_detect(OM_Name, "Grwth15"))
ems_exp2 <- read_xlsx(here("input", "run_EMs.xlsx"), sheet = "EM_Exp2", na = "NA") %>% 
  filter(!str_detect(EM_Name, "Sel")) %>% 
  mutate(EM_Name = paste(EM_Name, "Grwth_Misspecf")) # rename some EMs to explicitly state misspecification

# Growth Mis-specification ------------------------------------------------
...
for(n_om in 1:nrow(oms_exp2)) {
  ...
  om_scenario = here(om_path, oms_exp2$OM_Name[n_om])
  ...
  for(n_em in 1:nrow(ems_exp2)) {
    ...
    sim_models <- foreach(sim = 1:n_sims, .packages = c("TMB", "here", "tidyverse")) %dopar% {
      ...
      # Reverse sexes and age-length transition here (females = males, males = females)
      biologicals$waa_sex[,c(1,2)] <- biologicals$waa_sex[,c(2,1)]
      biologicals$al_matrix_sexsp[,,c(1,2)] <- biologicals$al_matrix_sexsp[,,c(2,1)]
      # Now assign it to another object for use in em_inputs
    ...
    # Progress
    cat(crayon::yellow("EM", n_em, "out of", nrow(ems_exp2)))
    
  } # end em loop
  
  # Progress
  cat(crayon::yellow("OM", n_om, "out of", nrow(oms_exp2)))
