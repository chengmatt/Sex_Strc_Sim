...
for(sim in 1:2) {
  ...
  InitDevs[,sim] = exp(rnorm(length(age_bins)-1, mean = -sigma_rec^2/2, sd = sigma_rec))
  
  for(s in 1:n_sexes) {
    # Get numbers at age - not plus group
    NAA[1,1:(length(age_bins)-1),s,sim] = r0 * exp(-M[s] * 0:(length(age_bins)-2)) * InitDevs[,sim] * sexRatio[s]
