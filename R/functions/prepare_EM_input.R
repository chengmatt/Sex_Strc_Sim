...
prepare_EM_inputs = function(sim,
  ...
  if(srv_age_prop == "across" & use_srv_sexRatio == TRUE) stop("No need to fit sex ratios when comps are parameterized across")
  if(fish_age_prop == "across" & use_fish_sexRatio == TRUE) stop("No need to fit sex ratios when comps are parameterized across")
  ...
  if(sex_specific == TRUE & n_sexes > 1) data$obs_fish_len_comps = array(Fish_LenComps[-n_years,,,,sim], dim = c(n_years-1, length(len_bins), n_sexes, n_fish_fleets))
  ...
  if(sex_specific == TRUE & n_sexes > 1) data$obs_srv_len_comps = array(Srv_LenComps[-n_years,,,,sim], dim = c(n_years-1, length(len_bins), n_sexes, n_fish_fleets))
  ...
  parameters$ln_meanF = rep(log(0.1), n_fish_fleets)
  ...
  if(selex_type == "length") {
    parameters$ln_fish_selpars = array(log(c(0.85, 60)), dim = c(n_fish_fleets, 2)) # only for logistic (last dim = number of selex pars)
    parameters$ln_srv_selpars = array(log(c(0.45, 52.5)), dim = c(n_srv_fleets, 2))  # only for logistic (last dim = number of selex pars)
