...
Type objective_function<Type>::operator() ()
{
    ...
    for(int f = 0; f < n_fish_fleets; f++) {
      for(int s = 0; s < n_sexes; s++) {
        ...
        for(int a = 0; a < n_ages; a++) {
          ...
          //   Type Total_fish_across_sex_tmp = sum(Total_Fish_Age_Numbers.col(f).transpose().col(y));
          //   for(int s = 0; s < n_sexes; s++) {
          //     for(int a = 0; a < n_ages; a++) {
          //       pred_fish_age_comps(y,a,s,f) /= Total_fish_across_sex_tmp;
          //     } // a loop
          //   } // s loop
  ...
  for(int f = 0; f < n_fish_fleets; f++) {
    ...
    for(int y = 0; y < n_years; y ++) {
      ...
      catch_nLL(y, f) -= use_catch(y,f) * dnorm(log(obs_catch(y, f)), 
                log(pred_catch(y,f)) - pow(catch_sd(f), 2)/2, catch_sd(f), true);
    } // fish fleet loop
  } // year loop
  ...
  vector<Type> obs_fish_age_as(n_years * n_sexes);
  vector<Type> pred_fish_age_as(n_years * n_sexes);
  ...
  for(int f = 0; f < n_fish_fleets; f++) {
    for(int y = 0 ; y < n_years; y++) {
      ...
      // Extract quantities
      // obs_fish_age_as = obs_fish_age_comps.col(f).transpose().col(y).transpose(); // Pull out observed vector
      // pred_fish_age_as = pred_fish_age_comps.col(f).transpose().col(y).transpose(); // Pull out observed vector
  ...
  REPORT(obs_fish_age_as);
  REPORT(pred_fish_age_as);
