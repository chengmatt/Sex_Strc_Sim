...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_ARRAY(obs_catch); // Array of catch from each fleet; n_years x n_sexes x n_fish_fleets (can fit sex-specific catch)
  ...
  array<Type> pred_catch(obs_catch.dim); // Predicted fishery catches
  ...
  array<Type> CAL(n_years, n_lens, n_sexes, n_fish_fleets); // Catch-at-age
    ...
    for(int y = 0; y < n_years; y++) {
      for(int s = 0; s < n_sexes; s++) {
        for(int a = 0; a < n_ages; a++) {
          ...
          CAA(y,a,s,f) = NAA(y,a,s) * (Type(1.0) - exp(-ZAA(y,a,s))) * (FAA(y,a,s,f) / ZAA(y,a,s));
  ...
  for(int f = 0; f < n_fish_fleets; f++) {
    ...
    for(int y = 0; y < n_years; y ++) {
      // Get likelihood here
      catch_nLL(y, f) -= dnorm(log(obs_catch(y, f)), log(pred_catch(y, f)) - pow(catch_sd(f), 2)/2, 
                         catch_sd(f), true);
      SIMULATE{ // Simulate catch
        obs_catch(y, f) = exp(rnorm(log(pred_catch(y, f)) - pow(catch_sd(f), 2)/2, catch_sd(f) ));
  ...
  for(int f = 0; f < n_fish_fleets; f++) {
    ...
    for(int y = 0 ; y < n_years; y++) {
      // Get likelihood here
      fish_index_nLL(y,f) -= dnorm(log(obs_fish_index(y, f)), 
                                   log(pred_fish_index(y, f)) - pow(fish_index_sd(f),2)/2, 
                                   fish_index_sd(f), true);
    
      SIMULATE{ // Simulate Fishery Index
        obs_fish_index(y, f) = exp(rnorm(log(pred_fish_index(y, f)) 
                              -pow(fish_index_sd(f),2)/2, fish_index_sd(f))); 
      } // Simulation statement
    } // y loop
  } // f loop
  ...
  for(int sf = 0; sf < n_srv_fleets; sf++) {
    ...
    for(int y = 0 ; y < n_years; y++) {
      // Get likelihood here
      srv_index_nLL(y,sf) -= dnorm(log(obs_srv_index(y, sf)),
                     log(pred_srv_index(y, sf)) - pow(srv_index_sd(sf),2)/2,
                     srv_index_sd(sf), true);

      SIMULATE{ // Simulate Survey Index
        obs_srv_index(y,sf) = exp(rnorm(log(pred_srv_index(y, sf))
                              -pow(srv_index_sd(sf),2)/2, srv_index_sd(sf)));
      } // Simulation statement
    } // y loop
  } // sf loop
